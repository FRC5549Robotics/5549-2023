...
public class FourConeAuto extends SequentialCommandGroup {
  ...
  public FourConeAuto(DrivetrainSubsystem drivetrainSubsystem, Intake intake, Telescope telescope, Tower tower, Limelight limelight, Claw claw, XboxController RumController) {
    ...
    addCommands(
      ...
      new ParallelCommandGroup(
        new Retract(m_telescope),
        m_drivetrainSubsystem.followTrajectoryCommand(RobotContainer.CT3ToBot),
        new RunIntakeAuto(m_intake)
      ),
      new ParallelCommandGroup(
        new ExtendMedium(m_telescope, rumController),
